...
import type { WeeklyScheduleRow } from "@/lib/fest-pass/scheduleSnapshot"
...
function FestPassButton({
  size = "default",
  label = "Claim my $49 Fest Pass",
}: {
  size?: "default" | "large"
  label?: string
}) {
  ...
  const handleCheckout = async () => {
    ...
    setError(null)
    try {
      const response = await fetch("/api/stripe/fest-pass-checkout", {
        method: "POST",
    ...
    return (
      <div className="border-b border-green-200 bg-green-50 px-4 py-4 text-center">
        <p className="text-base font-semibold text-green-800">
          Payment received — welcome to the fest pass! Check your email for confirmation. We&apos;ll see you on the
          dance floor.
        </p>
      </div>
    )
  ...
  return (
    <div className="overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-sm">
      <div className="hidden md:block">
        {schedule.map((row) => (
          <div key={row.day} className="grid grid-cols-[minmax(7rem,1fr)_1fr] border-b border-gray-100 last:border-b-0">
            
              <div className="bg-gray-50 px-4 py-3 font-bold text-gray-900">{row.day}</div>
              <div className="px-4 py-3">
                <ul className="space-y-2">
                  {row.slots.map((slot) => (
                    <li key={`${row.day}-${slot.time}-${slot.label}`} className="flex flex-wrap gap-x-2 text-gray-700">
...
export default function PuertoRicanFestContent({ schedule }: { schedule: WeeklyScheduleRow[] }) {
  return (
    <div className="min-h-screen bg-white">
      ...
      <section className="relative overflow-hidden bg-gradient-to-br from-[#1a1a2e] via-[#2d1b3d] to-[#FF3366]/35 text-white">
        ...
        <div className="container relative z-10 px-4 py-14 md:py-20">
          <p className="text-xs font-semibold uppercase tracking-[0.25em] text-white/75">Puerto Rican Fest · Studio E</p>
          <h1 className="mt-4 max-w-3xl text-3xl font-black leading-tight sm:text-4xl md:text-5xl lg:text-[2.75rem]">
            Puerto Rican Fest Dance Pass – 30 Days of Salsa &amp; Bachata for $49
          </h1>
      ...
      >
        <div className="container px-4 text-center">
          ...
          <div className="mt-10 flex flex-col items-center">
            <FestPassButton size="large" label="Checkout" />
