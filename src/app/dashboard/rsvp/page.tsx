  ...
  useEffect(() => {
    async function fetchRSVPs() {
      ...
      try {
        ...
        const { data: upcomingClassInquiries, error: upcomingClassesError } = await supabase
        ...
        // For each class inquiry, fetch the class details
        const upcomingClassesWithDetails: ClassRSVP[] = [];
        
        for (const inquiry of upcomingClassInquiries || []) {
          ...
          upcomingClassesWithDetails.push({
            ...inquiry,
            start_time: classData?.start_time || 'TBA',
            location: classData?.location || 'Location TBA'
          });
        }
        
        setUpcomingClasses(upcomingClassesWithDetails);
        
        // Fetch past classes
        const { data: pastClassInquiries, error: pastClassesError } = await supabase
          .from('class_inquiry_status')
          .select('*')
          .eq('user_id', user.id)
        ...
        // For each class inquiry, fetch the class details
        const pastClassesWithDetails: ClassRSVP[] = [];
        
        for (const inquiry of pastClassInquiries || []) {
          const { data: classData } = await supabase
            .from('classes')
            .select('start_time, location')
            .eq('id', inquiry.class_id)
        ...
        setPastClasses(pastClassesWithDetails);
        
        // Fetch upcoming events
        const { data: upcomingEventRSVPs, error: upcomingEventsError } = await supabase
        ...
        // For each event RSVP, fetch the event details
        const upcomingEventsWithDetails: EventRSVP[] = [];
        
        for (const rsvp of upcomingEventRSVPs || []) {
          ...
          upcomingEventsWithDetails.push({
            ...rsvp,
            start_time: eventData?.start_time || 'TBA',
        ...
        // Fetch past events
        const { data: pastEventRSVPs, error: pastEventsError } = await supabase
          .from('event_rsvp_status')
          .select('*')
          .eq('user_id', user.id)
          .eq('temporal_status', 'past');
        ...
        // For each event RSVP, fetch the event details
        const pastEventsWithDetails: EventRSVP[] = [];
        
        for (const rsvp of pastEventRSVPs || []) {
          const { data: eventData } = await supabase
            .from('events')
            .select('start_time, location')
            .eq('id', rsvp.event_id)
        ...
        setPastEvents(pastEventsWithDetails);
