    ...
    async function fetchData() {
      try {
        ...
        if (classesData) {
          ...
          // Get sorted dance styles with 'Other' at the end (replaced with fixed order)
          // We want a specific order: Salsa, Bachata, Heels, Choreo, Other
          const styleArray = ['Salsa', 'Bachata', 'Heels', 'Choreo', 'Other'].filter(style => styles.has(style));
  ...
  const applyInitialFilters = (classData: Class[]) => {
    ...
    if (danceStyle !== 'all') {
      filtered = filtered.filter(classItem => {
        ...
        if (danceStyle === 'Other') {
          // Other means it doesn't fit into the above categories
              ...
              <div className="grid gap-6 md:grid-cols-3 lg:grid-cols-4">
                ...
                <div>
                  <label className="mb-2 block text-sm font-medium">
                    Saved Instructors First
                  </label>
                  <div className="flex items-center space-x-2">
                    <input 
                      type="checkbox" 
                      id="saved-instructors" 
                      className="h-4 w-4 text-[#F94C8D] border-gray-300 focus:ring-[#F94C8D] rounded"
                      checked={false} // Static for now
                      onChange={() => {}} // Static for now
                    />
                    <label htmlFor="saved-instructors" className="text-sm">
                      Show Saved Instructors First
                    </label>
                  </div>
                </div>
                        ...
                        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                            ...
                            <div className="flex flex-col md:flex-row md:items-start gap-3 md:gap-6">
                              ...
                              <div className="flex flex-col items-center md:items-start min-w-[120px]">
                                <div className="text-lg font-bold text-gray-900">
                                  {format(new Date(classItem.class_date), 'EEE, MMM d')}
                                </div>
                              ...
                              <div className="flex-1 min-w-0">
                                <div className="flex items-start justify-between mb-2">
                                  ...
                                  <div className="flex items-center gap-2 flex-shrink-0">
                                    {classItem.is_series_start && classItem.series_length ? (
                                      <Badge variant="outline" className="border-[#9D4EDD] text-[#9D4EDD] bg-[#9D4EDD]/5">
                                        Series ({classItem.series_length})
                                      </Badge>
                                    ) : classItem.is_drop_in === true || classItem.series_length === null ? (
                                      <Badge variant="outline" className="border-[#EC407A] text-[#EC407A] bg-[#EC407A]/5">
                                        Drop-in
                                      </Badge>
                                    ) : (
                                ...
                                <div className="flex items-center text-sm text-gray-600 mb-2">
                                  ...
                                  <span className="truncate">
                                    {classItem.company.name} • {classItem.company.address.split(',')[0]}
                                  </span>
                                </div>
                                
                                <div className="flex items-center gap-4">
                                  <div className="text-lg font-bold text-[#EC407A]">
                                    ${classItem.price}
                                  </div>
                          ...
                          <div className="flex flex-col sm:flex-row gap-2 md:flex-col md:items-end">
                            <Button 
                              size="sm"
                              variant="outline"
                              className="border-[#F94C8D] text-[#F94C8D] hover:bg-[#F94C8D]/10 min-w-[80px]"
                              onClick={() => {
                            ...
                            <RSVPButton
                              ...
                              buttonClassName="bg-[#F94C8D] text-white hover:bg-[#F94C8D]/90 min-w-[80px]"
                            />
                          </div>
                        </div>
                      </div>
