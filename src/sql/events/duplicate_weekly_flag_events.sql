-- Duplicate Events with is_weekly=TRUE for the next 4 weeks
...
DECLARE
  ...
  weekly_events_cursor CURSOR FOR 
    SELECT * FROM "EVENT" 
    WHERE 
      -- Use the is_weekly flag to identify weekly events
      is_weekly = TRUE
...
BEGIN
  ...
  LOOP
    ...
    RAISE NOTICE 'Processing weekly event ID % - %', event_record.id, event_record.title;
    ...
    FOR weeks_offset IN 1..4 LOOP
      ...
      INSERT INTO "EVENT" (
        ...
        spots_left,
        cta_url,
        is_weekly  -- Include the is_weekly flag in new events
      ) VALUES (
        event_record.title,
        ...
        event_record.is_weekly  -- Keep the same value
      );
