...
import type { FaqItem } from "@/components/merch/MerchFaq"

type ReferralFaqProps = {
  items: FaqItem[]
}

export default function ReferralFaq({ items }: ReferralFaqProps) {
  ...
  return (
    <div className="divide-y divide-stone-200 rounded-2xl border border-stone-200 bg-white">
      {items.map((item, index) => {
        ...
        const panelId = `referral-faq-panel-${index}`
        const buttonId = `referral-faq-button-${index}`
